<template>
  <form class="meetup-form" @submit.prevent="$emit('submit', cloneDeep(localObject))">
    <div class="meetup-form__content">
      <fieldset class="meetup-form__section">
        <ui-form-group label="Название">
          <ui-input v-model="localObject.title" name="title" />
        </ui-form-group>
        <ui-form-group label="Дата">
          <ui-input-date v-model="localObject.date" type="date" name="date" />
        </ui-form-group>
        <ui-form-group label="Место">
          <ui-input v-model="localObject.place" name="place" />
        </ui-form-group>
        <ui-form-group label="Описание">
          <ui-input v-model="localObject.description" multiline rows="3" name="description" />
        </ui-form-group>
        <ui-form-group label="Изображение">
          ...
          <ui-image-uploader
            name="image"
            :preview="localObject.image"
            @select="localObject.imageToUpload = $event"
            @remove="localObject.imageToUpload = null"
          />
        </ui-form-group>
      </fieldset>
...
export default {
  ...
  data() {
    return {
      localObject: cloneDeep(this.meetup),
      cloneDeep,
    };
  },
  methods: {
    addAgendaItem() {
      const agenda = this.localObject.agenda;
      const time = agenda.length > 0 ? agenda[agenda.length - 1].endsAt : '00:00';
      this.localObject.agenda = [...agenda, { ...createAgendaItem(), startsAt: time }];
    },
    agendaItemRemove(id) {
